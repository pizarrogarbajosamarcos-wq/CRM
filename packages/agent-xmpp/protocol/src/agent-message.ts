...
export type BridgeWebhookPayload =
	| BridgeInboundPayload
	| BridgeFormResponsePayload;

export function isBridgeFormResponsePayload(
	payload: BridgeWebhookPayload,
): payload is BridgeFormResponsePayload {
...
export interface OutboundDeliverRequest {
	...
	from: string;
	to: string;
	/** BCP 47 language tag used as the stanza's inherited xml:lang. */
	lang?: string;
	threadId?: string | null;
	content: unknown;
	inReplyTo?: string;
