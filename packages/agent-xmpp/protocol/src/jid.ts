...
} from "precis-wasm/precis_wasm.js";
...
export function normalizeEndpointJid(value: string): string | null {
	...
	if (
		!local ||
		!domain ||
		LOCALPART_EXCLUDED.test(local) ||
		utf8Length(local) > 1023 ||
		utf8Length(domain) > 1023
	) {
		return null;
	}
	return `${local}@${domain}`;
...
function normalizeDomain(value: string): string | null {
	...
	if (
		!withoutFinalSeparator ||
		DOMAINPART_EXCLUDED.test(withoutFinalSeparator)
	) {
		return null;
	}
...
function normalizeIpLiteral(value: string): string | null | undefined {
	...
	const address =
		zoneDelimiter === -1 ? content : content.slice(0, zoneDelimiter);
	const zone =
		zoneDelimiter === -1 ? undefined : content.slice(zoneDelimiter + 3);
	if (isIP(address) !== 6 || (zone !== undefined && !IPV6_ZONE.test(zone)))
		return null;

	const hostname = new URL(`http://[${address}]/`).hostname.toLowerCase();
	if (zone === undefined) return hostname;
	const normalizedZone = zone.replace(/%[0-9A-Fa-f]{2}/gu, (encoded) =>
		encoded.toUpperCase(),
	);
