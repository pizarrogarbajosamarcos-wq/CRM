...
export function parseStrictJson(
	...
	if (utf8Length(text) > resolved.maxBytes)
		throw new JsonResourceLimitError("JSON payload exceeds byte limit");
	let offset = 0;
	let members = 0;
			...
			if (character === '"') {
				if (utf8Length(result) > resolved.maxStringBytes) {
					throw new JsonResourceLimitError(
						`JSON string exceeds byte limit at JSON offset ${offset}`,
					);
				}
	...
	const parseValue = (depth: number): unknown => {
		if (depth > resolved.maxDepth) {
			throw new JsonResourceLimitError(
				`JSON nesting exceeds depth limit at JSON offset ${offset}`,
			);
		}
		whitespace();
		const character = text[offset];
		...
		if (character === "{") {
			...
			while (true) {
				...
				if (++members > resolved.maxMembers) {
					throw new JsonResourceLimitError(
						`JSON member count exceeds limit at JSON offset ${offset}`,
					);
				}
				whitespace();
...
function assertJsonLimits(limits: StrictJsonLimits): void {
	for (const [name, value] of Object.entries(limits)) {
		if (!Number.isSafeInteger(value) || value < (name === "maxDepth" ? 0 : 1)) {
			throw new Error(
				`${name} must be a ${name === "maxDepth" ? "non-negative" : "positive"} safe integer`,
			);
		}
	}
}
