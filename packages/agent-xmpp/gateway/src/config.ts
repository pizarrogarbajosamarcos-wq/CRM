...
function envLanguageTag(name: string): string | undefined {
	...
	return value.length <= 64 && /^[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*$/.test(value)
		? value
		: undefined;
}
...
export function loadConfig(): GatewayConfig {
	...
	const agentDomain =
		process.env.XMPP_AGENT_DOMAIN ||
		componentJid.split(".").slice(1).join(".") ||
		componentJid;
	const inferredServerDomain =
		componentJid.split(".").slice(1).join(".") || componentJid;
	const reconnectInitialMs = envPosInt("XMPP_RECONNECT_INITIAL_MS", 1_000);
	const reconnectMaxMs = Math.max(
		reconnectInitialMs,
		envPosInt("XMPP_RECONNECT_MAX_MS", 60_000),
	);
	return {
		...
		defaultAgentJid:
			process.env.XMPP_DEFAULT_AGENT_JID || `assistant@${agentDomain}`,
