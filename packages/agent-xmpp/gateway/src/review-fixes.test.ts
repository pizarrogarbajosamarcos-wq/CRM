...
describe("review regressions", () => {
	...
	it("rejects invalid and foreign registration manifests", () => {
		const invalid = xml(
			...
			xml(
				...
				xml(
					"manifest",
					{ xmlns: AGENT_API_NS, "media-type": "application/json" },
					"{}",
				),
			),
		);
		const foreign = xml(
			...
			xml(
				...
				xml(
					"manifest",
					{ xmlns: "urn:example:foreign", "media-type": "application/json" },
					"{}",
				),
			),
		);
	...
	it("rejects non-object task result and event payloads", () => {
		...
		expect(() => parseTaskResult(result)).toThrow(
