...
import { bareJid } from "@agent-xmpp/protocol";
	...
	constructor(public readonly response: Element) {
		...
		const condition = stanzaError?.children.find(
			(child): child is Element =>
				typeof child !== "string" && child.name !== "text",
		);
...
const DEFAULT_MAX_PENDING_IQ_REQUESTS = 256;
...
export function buildIqError(request: Element): Element {
	return xml(
		...
		xml(
			"error",
			{ type: "cancel" },
			xml("service-unavailable", { xmlns: STANZA_ERROR_NS }),
		),
	);
}

function iqMiddlewareReply(response: Element): Element | true {
	if (response.attrs.type === "error") {
		return (
			response.getChild("error") ??
			xml(
				"error",
				{ type: "cancel" },
				xml("service-unavailable", { xmlns: STANZA_ERROR_NS }),
	...
	return response.getChildElements()[0] ?? true;
}
 ...
 */
export async function dispositionForStanza(
	stanza: Element,
	onIqGet?: IqGetHandler,
): Promise<IqDisposition> {
	if (stanza.name === "iq") {
		const type = String(stanza.attrs.type ?? "");
		if (type === "get" || type === "set") {
			const response = (await onIqGet?.(stanza)) ?? null;
			return response
				? { kind: "respond", stanza: response }
				: { kind: "error" };
		}
	}
...
export function reconnectDelayMs(
	...
	const exponential = Math.min(
		maxMs,
		initialMs * 2 ** Math.min(Math.max(attempt - 1, 0), 30),
	);
...
export function createComponentSession(
	...
	onIqGet?: IqGetHandler,
): XmppComponentSession {
	const maxPendingIqRequests =
		config.maxPendingIqRequests ?? DEFAULT_MAX_PENDING_IQ_REQUESTS;
	if (
		!Number.isSafeInteger(maxPendingIqRequests) ||
		maxPendingIqRequests <= 0
	) {
		throw new Error("maxPendingIqRequests must be a positive integer");
	}
	...
	const settleIqRequest = (
		id: string,
		responseOrError: Element | Error,
	): boolean => {
		const pending = pendingIqRequests.get(id);
		if (!pending) return false;
		if (!(responseOrError instanceof Error)) {
			const responseFrom = bareJid(String(responseOrError.attrs.from ?? ""));
			const responseTo = bareJid(String(responseOrError.attrs.to ?? ""));
			if (
				(pending.expectedFrom && responseFrom !== pending.expectedFrom) ||
				(pending.expectedTo && responseTo !== pending.expectedTo)
			) {
				return false;
			}
		}
		...
		else if (responseOrError.attrs.type === "error")
			pending.reject(new IqResponseError(responseOrError));
	...
	const scheduleReconnect = (): void => {
		...
		console.error(
			`[xmpp-gateway] reconnect attempt ${reconnectAttempt} scheduled in ${delay}ms`,
		);
	...
	const createClient = (): ReturnType<typeof component> => {
		...
		// @xmpp/component installs its IQ callee as middleware. Handling IQs only
		// from the raw `stanza` event races that callee: it immediately emits
		// service-unavailable while our listener later emits the valid result.
		// Participate in the middleware chain so every request gets exactly one
		// response.
		const middlewareClient = client as typeof client & {
			middleware: {
				use(
					handler: (
						context: { stanza: Element },
						next: () => Promise<unknown>,
					) => Promise<unknown>,
				): unknown;
			};
		};
		middlewareClient.middleware.use(async (context, next) => {
			const stanza = context.stanza as Element;
			const type = String(stanza.attrs.type ?? "");
			if (stanza.name !== "iq" || (type !== "get" && type !== "set"))
				return next();
			if (activeInboundIq >= maxPendingIqRequests) {
				return xml(
					"error",
					{ type: "wait" },
					xml("resource-constraint", { xmlns: STANZA_ERROR_NS }),
				);
			}
			activeInboundIq++;
			try {
				const disposition = await dispositionForStanza(stanza, onIqGet);
				const response =
					disposition.kind === "respond"
						? disposition.stanza
						: buildIqError(stanza);
				return iqMiddlewareReply(response);
			} catch (err) {
				console.error("[xmpp-gateway] inbound IQ handling failed:", err);
				return iqMiddlewareReply(buildIqError(stanza));
			} finally {
				activeInboundIq--;
			}
		});
		...
		client.on("stanza", (stanza: Element) => {
			...
			// Correlate outbound requests before any inbound protocol routing.
			if (
				stanza.name === "iq" &&
				id &&
				(type === "result" || type === "error") &&
				settleIqRequest(id, stanza)
			) {
				return;
			}

			if (stanza.name === "iq" && (type === "get" || type === "set")) {
				// The @xmpp/component IQ middleware above owns the response.
				return;
			}
		...
		client.on("disconnect", () =>
			handleConnectionLoss(client, "component disconnected"),
		);
		client.on("offline", () =>
			handleConnectionLoss(client, "component went offline"),
		);
		return client;
	};
	...
	async function connectClient(): Promise<void> {
		...
		try {
			...
			// A disconnect can reject this while client.open() is still pending.
			// Handle that timing window immediately; awaiting the original promise
			// below still propagates the rejection.
			void onlinePromise.catch(() => undefined);
			try {
			...
			if (activeClient === client && !stopped) {
				...
				transition("online");
				console.error(
					`[xmpp-gateway] component online: ${config.componentJid}`,
				);
			}
		} catch (error: unknown) {
	...
	const send = async (stanza: Element): Promise<void> => {
		...
		if (state !== "online" || !client)
			throw new Error("XMPP component is offline");
	...
	const requestIq = (
		...
		if (state !== "online")
			return Promise.reject(
				new Error("Cannot send IQ request while component is offline"),
			);
		...
		if (stanza.name !== "iq" || (type !== "get" && type !== "set")) {
			return Promise.reject(
				new Error(
					'Outbound IQ request must be an <iq type="get"> or <iq type="set"> stanza',
				),
			);
		}
		...
		if (
			...
			return Promise.reject(
				new Error("IQ request timeoutMs must be a positive integer"),
			);
		}
		if (pendingIqRequests.size >= maxPendingIqRequests) {
			return Promise.reject(
				new Error(
					`Too many pending IQ requests (limit ${maxPendingIqRequests})`,
				),
			);
		}
		...
		if (pendingIqRequests.has(id)) {
			return Promise.reject(
				new Error(`IQ request id is already pending: ${id}`),
			);
		}
		stanza.attrs.id = id;
		...
		return new Promise<Element>((resolve, reject) => {
			const timer = setTimeout(() => {
				settleIqRequest(
					id,
					new Error(`IQ request ${id} timed out after ${timeoutMs}ms`),
				);
			}, timeoutMs);
			timer.unref?.();

			const pending: PendingIqRequest = {
				resolve,
				reject,
				timer,
				signal: options.signal,
				expectedFrom: bareJid(String(stanza.attrs.to ?? "")),
				expectedTo: bareJid(String(stanza.attrs.from ?? "")),
			};
			if (options.signal) {
				pending.onAbort = () => settleIqRequest(id, abortError(id));
				options.signal.addEventListener("abort", pending.onAbort, {
					once: true,
				});
			}
