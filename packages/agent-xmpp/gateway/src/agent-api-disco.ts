...
function assertPayloadShape(payload: Element, shape: PayloadShape): void {
	...
	if (
		required.some(
			(name) => payload.attrs[name] === undefined || payload.attrs[name] === "",
		) ||
	...
	if (
		...
		actualChildren.some(
			(child, index) =>
				...
				(expectedChildren[index]!.xmlns !== undefined &&
					child.attrs.xmlns !== expectedChildren[index]!.xmlns),
		)
	) {
...
function requestPayload(
	...
	if (request.attrs.type !== expectedIqType)
		throw new Error(`${name} requires an IQ of type ${expectedIqType}`);
	return payload;
}
...
export function parseManifestRequest(
	...
	const version =
		payload.attrs.version === undefined
			? undefined
			: String(payload.attrs.version);
	if (version !== undefined && !isApiVersion(version))
		throw new Error("manifest-request has an invalid version");
...
function resultForm(formType: string, fields: Element[]): Element {
	return xml(
		...
		field("FORM_TYPE", formType, "hidden"),
		...fields,
	);
}
	...
	return resultIq(
		...
		xml(
			...
			xml("identity", {
				category: "automation",
				type: "agent-gateway",
				name: "NanoClaw XMPP Agent Gateway",
			}),
			...features(
				DISCO_INFO_NS,
				DISCO_ITEMS_NS,
...
export function buildDirectoryInfo(
	request: Element,
	componentJid: string,
): Element {
	return resultIq(
...
export function buildAgentDirectory(
	...
	const page = pageRsm(
		agents,
		(agent) => agent.manifest.agent.jid,
		parseRsm(query),
	);
	return resultIq(
		request,
		...
		xml(
			"query",
			{
				xmlns: DISCO_ITEMS_NS,
				...(query.attrs.node ? { node: query.attrs.node } : {}),
			},
...
export function buildToolItems(
	...
	return resultIq(
		...
		xml(
			...
			{
				xmlns: DISCO_ITEMS_NS,
				node: toolsNode(agent.manifest.agent.version, namespaces),
			},
			...page.items.map((tool) =>
				xml("item", {
					jid: agent.manifest.agent.jid,
...
export function buildToolInfo(
	...
	): Element[] => (value === undefined ? [] : [field(name, value ? "1" : "0")]);
	return resultIq(
		request,
		agent.manifest.agent.jid,
		xml(
			...
			resultForm(namespaces.toolInfo, [
				...
				...(tool.outputSchemaHash
					? [
							field("output_schema_hash_algo", "sha-256"),
							field("output_schema_hash_value", tool.outputSchemaHash),
						]
					: []),
...
export function buildSchemaResult(
	...
	const schemaHash =
		direction === "input" ? tool.inputSchemaHash : tool.outputSchemaHash;
...
export function parseManifestRegistration(
	...
	const manifest = request
		.getChild("register", namespaces.api)
		?.getChild("manifest", namespaces.api);
	if (!manifest || manifest.attrs["media-type"] !== JSON_MEDIA_TYPE)
		return null;
	try {
		return validateManifest(
			parseStrictJson(manifest.getText(), { maxBytes: 1_048_576 }),
		);
	} catch (error) {
