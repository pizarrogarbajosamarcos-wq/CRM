...
export type XmppComponentSessionFactory = (
 ...
 */
export function createEmbeddedIqHandler(
	options: EmbeddedIqHandlerOptions,
	downstream?: IqGetHandler,
): IqGetHandler {
	return async (stanza) => {
		...
		if (
			...
			to === bareJid(options.componentJid) &&
			info &&
			!info.attrs.node
		) {
			return buildGatewayInfo(
				stanza,
				options.componentJid,
				options.protocolNamespaces ?? DEFAULT_PROTOCOL_NAMESPACES,
			);
		}
...
export class EmbeddedXmppGateway {
	...
	private connectionState: ReturnType<XmppComponentSession["getState"]> =
		"offline";
	private readonly presenceStore: PresenceSubscriptionStore;
	private readonly publishedPresence = new Map<
		string,
		{ agent: VirtualAgentIdentity; subscriberJid: string }
	>();
	...
	async start(): Promise<void> {
		...
		const createSession =
			this.dependencies.componentSessionFactory ?? createComponentSession;
		const session = createSession(
			this.config,
			createEmbeddedIqHandler(this.config, this.dependencies.onIqGet),
		);
		const sendForAgent = async (_agentJid: string, stanza: Element) =>
			session.send(stanza);
		const router = new StanzaRouter(
			...
			(agent, subscriberJid, subscribed) =>
				this.updatePresenceSubscription(agent, subscriberJid, subscribed),
		);
		...
		this.keepalive = new XmppKeepalive(
			{
				...
				failureThreshold: this.config.pingFailureThreshold,
			},
			{
				...
				ping: async () => {
					await session.requestIq(
						xml(
							...
							{
								type: "get",
								from: this.config.componentJid,
								to: this.config.serverDomain,
							},
	...
	private updatePresenceSubscription(
		...
		this.presenceStore.setPresenceSubscription(
			route.agentJid,
			route.subscriberJid,
			subscribed,
		);
	...
	private async reconcilePresence(
		...
		for (const subscription of this.presenceStore.listPresenceSubscriptions()) {
			const agent = this.dependencies.resolveVirtualAgent?.(
				bareJid(subscription.agentJid),
			);
			if (!agent) continue;
			const route = {
				agent,
				subscriberJid: bareJid(subscription.subscriberJid),
			};
		...
		for (const [key, route] of this.publishedPresence) {
			...
			await session.send(
				buildUnavailablePresence(route.agent, route.subscriberJid),
			);
		...
		for (const [key, route] of desired) {
			...
			await session.send(
				buildAvailablePresence(route.agent, route.subscriberJid),
			);
			this.publishedPresence.set(key, route);
		}
	}

	private async publishUnavailablePresence(
		session: XmppComponentSession,
	): Promise<void> {
		for (const route of this.publishedPresence.values()) {
			await session
				.send(buildUnavailablePresence(route.agent, route.subscriberJid))
				.catch((err) => {
					console.error(
						"[xmpp-gateway] unavailable presence send failed:",
						err,
					);
				});
		}
	...
	async deliver(
		input: OutboundDeliverRequest & { from: string },
	): Promise<string> {
		const built = buildOutboundStanza(
			{ ...input, lang: input.lang ?? this.config.xmlLang },
			input.from,
		);
		// XEP-0334 <store/> so an offline 1:1 peer still gets it; MUC messages aren't stored.
		const stanza = applyStoreHints(
			built,
			built.attrs.type === "chat" ? { store: true } : undefined,
		);
