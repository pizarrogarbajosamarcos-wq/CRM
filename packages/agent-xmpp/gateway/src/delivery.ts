...
import {
	isMentionForAgent,
	shouldDeliverInbound,
} from "./xep-plugins/routing.js";
...
export function shouldAcceptStanza(
	stanzaType: string,
	from: string,
	bodyText: string,
	agentNick: string,
...
export function buildBridgePayload(
	...
	const {
		...
		threadId,
		groupchat: isGroup,
	} = resolveInboundChatTargets(from, stanzaType, agentMsg);
...
export function buildFormResponsePayload(
	...
	const {
		...
		threadId,
		groupchat: isGroup,
	} = resolveInboundChatTargets(ctx.from, ctx.stanzaType, {
		from: ctx.from,
		threadId: undefined,
	});
