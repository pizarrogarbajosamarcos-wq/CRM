...
export function parseTaskInvocation(
	...
	const expectedNames =
		children.length === 3
			? ["manifest-hash", "arguments", "deadline"]
			: ["manifest-hash", "arguments"];
	if (
		children.length < 2 ||
		children.length > 3 ||
		children.some(
			(child, index) =>
				child.name !== expectedNames[index] ||
				child.getNS() !== namespaces.task,
		)
	) {
		throw new Error(
			"invoke children must be manifest-hash, arguments, then optional deadline",
		);
	}
	...
	if (
		hashChildren.length !== 1 ||
		hashChildren[0]!.name !== "hash" ||
		hashChildren[0]!.getNS() !== namespaces.hashes
	) {
		throw new Error("manifest-hash must contain exactly one XEP-0300 hash");
	}
	assertOnlyAttributes(hashChildren[0]!, ["xmlns", "algo"]);
	if (
		argumentsElement!.getChildElements().length > 0 ||
		(deadlineElement?.getChildElements().length ?? 0) > 0
	) {
...
export function parseTaskCancellation(
	...
	if (
		children.length > 1 ||
		children.some(
			(child) => child.name !== "reason" || child.getNS() !== namespaces.task,
		)
	) {
		throw new Error("cancel may contain only one reason");
	}
	if (
		cancel.children.some(
...
export function parseTaskInput(
	...
	if (
		input.attrs["media-type"] !== JSON_MEDIA_TYPE ||
		input.getChildElements().length > 0
	) {
	...
	const expectedRevision = parseNonNegativeInteger(
		provide.attrs["expected-revision"],
	);
	if (!isOpaqueId(taskId) || !isOpaqueId(requestId))
		throw new Error("invalid task input identifier");
	return {
		taskId,
		requestId,
		expectedRevision,
		input: parseStrictJson(input.getText()),
	};
}
...
export function parseAcceptedResult(
	...
	if (!Number.isSafeInteger(revision) || revision < 0)
		throw new Error("accepted task has invalid revision");
	...
	return xml(
		...
		xml(
			"task-state",
			{
				...
				"result-available": terminalTaskStates.has(task.state)
					? "true"
					: "false",
			...
			xml(
				...
				xml(
					"hash",
					{ xmlns: namespaces.hashes, algo: "sha-256" },
					task.manifestHash,
				),
			),
			...(task.pendingInput
				? [
						xml(
							"pending-input",
							{ "media-type": JSON_MEDIA_TYPE },
							JSON.stringify(task.pendingInput),
						),
					]
				: []),
		),
...
export function buildTaskStateRequest(
	task: AgentTaskRecord,
	remoteTaskId: string,
): Element {
	return xml(
...
export function buildTaskResultRequest(
	task: AgentTaskRecord,
	remoteTaskId: string,
): Element {
	return xml(
...
export function parseTaskStateResult(
	stanza: Element,
	...
	if (
		...
		(state.attrs.deadline !== undefined &&
			!isXep0082DateTime(String(state.attrs.deadline)))
	) {
	...
	return {
		...
		pendingInput: pending
			? (parseStrictJson(pending.getText()) as PendingTaskInput)
			: undefined,
	};
}
...
export function parseTaskResult(stanza: Element): TaskResultSnapshot | null {
	if (stanza.name !== "iq" || stanza.attrs.type !== "result") return null;
	const result = stanza.getChild(
		"task-result",
		DEFAULT_PROTOCOL_NAMESPACES.task,
	);
...
export function parseTaskEvent(
	...
	if (events.length !== 1)
		throw new Error("task event message must contain exactly one event");
	const messageType = String(stanza.attrs.type ?? "normal");
	if (messageType !== "normal")
		throw new Error("invalid task event message type");
	const event = events[0]!;
	assertOnlyAttributes(event, [
		"xmlns",
		"task-id",
		"event-id",
		"revision",
	...
	if (event.getChildElements().length > 0)
		throw new Error("task event payload must contain character data only");
	...
	if (!Number.isSafeInteger(revision) || revision < 1)
		throw new Error("invalid task event revision");
	const taskId = String(event.attrs["task-id"] ?? "");
	const eventId = String(event.attrs["event-id"] ?? "");
	if (!isOpaqueId(taskId) || !isOpaqueId(eventId))
		throw new Error("invalid task event identifier");
	return {
		taskId,
		eventId,
		revision,
...
function assertIqRequest(
	...
	if (
		...
		throw new Error(
			`${payload.name} must be the only payload of an IQ ${type}`,
		);
	}
}
...
function parseNonNegativeInteger(value: unknown): number {
	if (typeof value !== "string" || !/^\+?\d+$/.test(value))
		throw new Error("missing non-negative integer");
