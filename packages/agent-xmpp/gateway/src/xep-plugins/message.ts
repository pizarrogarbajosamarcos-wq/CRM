...
function parseJsonPayload(
	stanza: Element,
): { kind: MessageKind; contentType: string; body: unknown } | null {
	const payload = stanza.getChild("payload", JSON_NS);
	if (!payload) return null;
...
export function stanzaToAgentMessage(
	stanza: Element,
	agentDomain: string,
): AgentMessage | null {
	...
	const threadId =
		threadEl?.getText()?.trim() || (threadEl?.attrs as { id?: string })?.id;
...
export function resolveTargetAgentJid(
	to: string,
	agentDomain: string,
	defaultAgent: string,
): string {
...
export function buildOutboundStanza(
	req: OutboundDeliverRequest,
	fromJid: string,
): Element {
	...
	if (req.inReplyTo) {
		...
		children.push(
			xml("reply", {
				xmlns: REPLY_NS,
				id: req.inReplyTo,
				to: isMuc ? req.to : bareJid(req.to),
			}),
		);
	}
	...
	return xml(
		"message",
		{
			...
			to,
			from: fromJid,
			...(req.lang ? { "xml:lang": req.lang } : {}),
		},
		...children,
	);
}

export function applyStoreHints(
	stanza: Element,
