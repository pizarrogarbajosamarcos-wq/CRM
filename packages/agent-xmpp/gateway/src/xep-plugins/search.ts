...
const INSTRUCTIONS =
	"Enter a nickname to search for matching agents. Leave it empty to list up to 100 agents.";
...
function resultIq(
	...
	return xml(
		"iq",
		{
			...
			...(request.attrs["xml:lang"]
				? { "xml:lang": request.attrs["xml:lang"] }
				: {}),
		},
		query,
	);
...
export function buildSearchResults(
	...
	const needle = (
		submittedForm
			? dataFormFieldValue(submittedForm, "nick")
			: (query?.getChildText("nick") ?? "")
	)
		.trim()
		.toLowerCase();
	const matches = agents.filter((agent) => {
		...
		return [localpart, identity.name, identity.title ?? ""].some((value) =>
	...
	const page = pageRsm(
		...
		parseRsm(query!, MAX_SEARCH_RESULTS),
	);
					...
					...page.items.map((agent) =>
						xml(
							...
							xml(
								"field",
								{ var: "jid" },
								xml("value", {}, agent.manifest.agent.jid),
							),
