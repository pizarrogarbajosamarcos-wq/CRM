...
export function buildJoinPresence(
	input: XmppJoinRoomInput,
	agentJid: string,
): Element {
	...
	return xml(
		"presence",
		{ to: roomWithNick, from: agentJid },
		xml("x", { xmlns: MUC_NS }, ...mucChildren),
	);
}

export function buildLeavePresence(
	input: XmppLeaveRoomInput,
	agentJid: string,
	nickname?: string,
): Element {
...
export function buildRoomMessage(
	input: XmppSendRoomMessageInput,
	fromJid: string,
): Element {
